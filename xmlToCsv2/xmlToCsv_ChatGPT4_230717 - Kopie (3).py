...
def build_header_dict(element, parent_map, parent_path="", header_dict=None, keyword_indexes=None, last_keyword_level=-1, currentKeyword = None):
    ...
    # Adjust the tag name and keyword index if necessary
    ...
    for attr in element.attrib:
        ...
        if currentKeyword:
            ...
            header_dict[(attrName, level)] = attr_path
        else:
            header_dict[(attr, level)] = attr_path

    # Recursively add children to the dictionary
    for child in element:
        build_header_dict(child, parent_map, full_tag_path, header_dict, keyword_indexes, last_keyword_level, currentKeyword)
            ...
            if ((header in data) or shortCategory) and (itemLevel == header_level):
                ...
                if(header != itemTag):
                    if(header == "RefID" and itemTag != "TypeRoot" and header_level == 3):
                        ...
                        if(currentKeyword):
                            full_tag_path = header_dict[f"{header}_{keyword_indexes[currentKeyword]}",header_level]
                        else:
                            full_tag_path = header_dict[header,header_level]
...
def navigate_to_element(element, tag_path_parts):
    ...
    for level,tag in enumerate(tag_path_parts,1):
        ...
        idx = int(tag_parts[1]) if len(tag_parts) > 1 else None

        if (actual_tag, level) in keywordsTuple:
            # We're looking for a keyword element
            
            
            # Loop through children until we find the correct instance of the keyword
            for child in current_element:
                if child.tag == actual_tag:
                    #We've found a keyword
                    keyword_counts[actual_tag] += 1
                    #Check if it is the one we are looking for
                    if keyword_counts[actual_tag] == idx:
                        current_element = child
                        break
                    
        else:
            # We're looking for a non-keyword element
            for child in current_element:
                if child.tag == actual_tag:
                    current_element = child
                    break
...
keywordsTuple = [("Layer",4), ("LineType",4), ("BuildingMaterial",4), ("CompositeWall",4), ("Material",4), ("Fill",4), ("Profile",4),("Attributes",3), ("PenTable", 4), ("Pens",5), ("Pen", 6)] #maybe delete? Probably not needed anymore
