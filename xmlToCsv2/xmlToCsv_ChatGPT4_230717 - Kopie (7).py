...
def build_header_dict(element, parent_map, parent_path="", header_dict=None, keyword_indexes=None, last_keyword_level=-1, currentKeyword = None , duplicate_indexes= None):
    ...
    if (elementTag, level, parent_tag, grandParent_tag) in duplicate_indexes:
        duplicate_indexes[(elementTag, level, parent_tag,grandParent_tag)] += 1
    else:
        duplicate_indexes[(elementTag, level, parent_tag,grandParent_tag)] = 0
    index = duplicate_indexes[(elementTag, level, parent_tag,grandParent_tag)]
    ...
    header_dict[(tag, level, parent_tag, index,grandParent_tag)] = full_tag_path
    ...
    for attr in element.attrib:
        ...
        if currentKeyword:
            ...
            header_dict[(attrName, level, tag, attr_index,parent_tag)] = attr_path
        else:
            header_dict[(attr, level, tag, attr_index,parent_tag)] = attr_path
...
def get_headers(root, parent_map): ####still some problems with saving all the headers - some branches have significantly more sub-branches, which have an impact on the headers amount too. The error might be here or with csv exporting. More troubleshooting necessary...
    ...
    for item in root.iter():
        ...
        for tag,level,pTag,gpTag,ggpTag,gggpTag in occurrence_counts.keys():
            if level > item_level:
                occurrence_counts[tag,level,pTag,gpTag,ggpTag,gggpTag] = 0
               

        # Increment the occurrence count for this tag, level, parent, and grandparent, and get the current count.
        occurrence_counts[(itemTag, item_level, parentTag,grandParentTag, gGrandParentTag,ggGrandParentTag)] = occurrence_counts.get((itemTag, item_level, parentTag, grandParentTag,gGrandParentTag,ggGrandParentTag), 0) + 1
        index = occurrence_counts[(itemTag, item_level, parentTag, grandParentTag, gGrandParentTag,ggGrandParentTag)] - 1

        # Add the item to its parent's list of children in children_dict.
        children_dict.setdefault((parentTag,grandParentTag,gGrandParentTag,ggGrandParentTag),[]).append((itemTag, item_level, parentTag, index, grandParentTag,gGrandParentTag,ggGrandParentTag))
        ...
        for attr in item.attrib.keys():
            ...
            children_dict.setdefault((itemTag,parentTag,grandParentTag,gGrandParentTag), []).append((attr, item_level, itemTag, attr_index, parentTag,grandParentTag,gGrandParentTag))
    ...
    def add_header(header):
        ...
        for child in children_dict.get((header[0],header[2], header[4],header[5]), []):
            #if((child[0], child[1]) not in keywordsTuple):
                add_header(child)

    for header in children_dict.get((None,None,None,None), []):
        add_header(header)
...
def get_first_row(headers, row, root,cell_path_matrix):
    ...
    for i, (header, header_level, header_parent, index, header_grandParent, header_gGrandParent,header_ggGrandParent) in enumerate(headers):  # Manual correction to export also attributes of root
        if header in attributes and header_level == 1:
            row[i] = (root.attrib[header])
            full_tag_path = header_dict[header,header_level, header_parent, index,header_grandParent]
...
def write_lines (headers, root, cell_path_matrix, writer, parent_map): ##### trying to correct write_lines by introducing greatgrandparents into header_dict, but now having problems with accessing the dictionary when writing stuff out. Logic might need some deep rethinking. I created keyword_ancestor_dict to simplify the process
    ...
    for item in root.findall('.//'):
        ...
        for i, (header, header_level, header_parent, header_index, header_grandParent, header_gGrandParent, header_ggGrandParent) in enumerate(headers):
            ...
            if (((header,header_parent) in data) or shortCategory) and (itemLevel == header_level):
                ...
                if(header != itemTag and header_parent == itemTag and header_grandParent == parentTag): #if we found attributes of item (which means level is the same as aprent)
                        ...
                        if(currentKeyword is not None): #if there is a keyword
                            if(parentAncestor is not None):#if parent also has  keyword
                                if(grandParentAncestor is not None):#if grandparent also has a keyword
                                    full_tag_path = header_dict[(f"{header}_{keyword_indexes[currentKeyword]}", header_level, f"{header_parent}_{keyword_indexes[parentAncestor]}", header_index,f"{header_grandParent}_{keyword_indexes[grandParentAncestor]}")]
                                else: # if grandparent does not have a keyword
                                    full_tag_path = header_dict[(f"{header}_{keyword_indexes[currentKeyword]}", header_level, f"{header_parent}_{keyword_indexes[parentAncestor]}", header_index,header_grandParent)]
                            else: # if neither parent, nor grandparent has a keyword
                                full_tag_path = header_dict[(f"{header}_{keyword_indexes[currentKeyword]}", header_level, header_parent, header_index,header_grandParent)]
                            #if(header_level != parentLevel):
                                #if(header_level-keywordLevel >= 1):  ##if it is under a keyword and grandparent is also already (under) the same keyword
                                    #full_tag_path = header_dict[(f"{header}_{keyword_indexes[currentKeyword]}", header_level, f"{header_parent}_{keyword_indexes[currentKeyword]}", header_index,f"{header_grandParent}_{keyword_indexes[currentKeyword]}")]
                                #else:
                                    #if(header_level - keywordLevel == 0):##if only parent is current keyword
                                        #if(grandParentAncestor is not None): ##if grandparent is different keyword
                                        #if (header_grandParent, (header_level+1)) in keywordsTuple: 
                                            #full_tag_path = header_dict[(f"{header}_{keyword_indexes[currentKeyword]}", header_level, f"{header_parent}_{keyword_indexes[currentKeyword]}", header_index,f"{header_grandParent}_{keyword_indexes[grandParentAncestor]}")]
                                        #else:  ##if grandparent has no keyword
                                            #full_tag_path = header_dict[(f"{header}_{keyword_indexes[currentKeyword]}", header_level, f"{header_parent}_{keyword_indexes[currentKeyword]}", header_index,header_grandParent)]
                                    #else: ## if item itself is keyword - not possible, because it is an attribute!
                                        #if(header_grandParent, (header_level-2)) in keywordsTuple:##if grandparent is different keyword
                                            #full_tag_path = header_dict[(f"{header}_{keyword_indexes[currentKeyword]}", header_level, f"{header_parent}_{keyword_indexes[header_grandParent]}", header_index,f"{header_grandParent}_{keyword_indexes[header_grandParent]}")]
                                        #else:
                                            #if(header_parent, (header_level-1)) in keywordsTuple:##if parent is different keyword
                                                #full_tag_path = header_dict[(f"{header}_{keyword_indexes[currentKeyword]}", header_level, f"{header_parent}_{keyword_indexes[header_parent]}", header_index,header_grandParent)]
                                            #else:
                                                #if(header_level != parentLevel): #if only item is keyword
                                                    #if(header, header_level) in keywordsTuple:
                                                        #full_tag_path = header_dict[(f"{header}_{keyword_indexes[currentKeyword]}", header_level, header_parent, header_index,header_grandParent)]
                                                    #else: ##if current element is an attribute and is on same level with keyword
                                                        #full_tag_path = header_dict[(f"{header}_{keyword_indexes[currentKeyword]}", header_level, f"{header_parent}_{keyword_indexes[header_parent]}", header_index,header_grandParent)]
                                        #continue
                        else: # if there are no keywords
                            #if(grandParentAncestorLevel is not None): #i'm not sure this is even possible
                                #full_tag_path = header_dict[(header, header_level, header_parent, index, f"{header_grandParent}_{keyword_indexes[grandParentAncestor]}")]
                           #else:
                                try:
                                    full_tag_path = header_dict[(header, header_level, header_parent, index, header_grandParent)]
                                except:
                    ...
                    if(header == itemTag and header_parent == parentTag and header_grandParent == grandParentTag):
                        try:
                        ...
                        try:
                            ...
                            if(currentKeyword is not None):
                                if(parentAncestor is not None):#if parent also has  keyword
                                    if(grandParentAncestor is not None):#if grandparent also has a keyword
                                        if(currentKeyword == header):
                                            full_tag_path = header_dict[(f"{header}_{keyword_indexes[currentKeyword]}", header_level, f"{header_parent}_{keyword_indexes[parentAncestor]}", index,f"{header_grandParent}_{keyword_indexes[grandParentAncestor]}")]
                                        else:
                                            full_tag_path = header_dict[(f"{header}_{keyword_indexes[currentKeyword]}", header_level, f"{header_parent}_{keyword_indexes[parentAncestor]}", header_index,f"{header_grandParent}_{keyword_indexes[grandParentAncestor]}")]
                                    else: # if grandparent does not have a keyword
                                        full_tag_path = header_dict[(f"{header}_{keyword_indexes[currentKeyword]}", header_level, f"{header_parent}_{keyword_indexes[parentAncestor]}", header_index,header_grandParent)]
                                ...
                                #if(header_level-keywordLevel >= 2):  ##if it is under a keyword and grandparent is also already (under) the same keyword
                                    #full_tag_path = header_dict[(f"{header}_{keyword_indexes[currentKeyword]}", header_level, f"{header_parent}_{keyword_indexes[currentKeyword]}", header_index,f"{header_grandParent}_{keyword_indexes[currentKeyword]}")]
                                #else:
                                    #if(header_level-keywordLevel == 1): #if grandparent is not (under) the same keyword
                                        #if(grandParentAncestor is not None): #if grandparent is has a different keyword
                                            #full_tag_path = header_dict[(f"{header}_{keyword_indexes[currentKeyword]}", header_level, f"{header_parent}_{keyword_indexes[currentKeyword]}", header_index,f"{header_grandParent}_{keyword_indexes[grandParentAncestor]}")]
                                        #else:
                                            #full_tag_path = header_dict[(f"{header}_{keyword_indexes[currentKeyword]}", header_level, f"{header_parent}_{keyword_indexes[currentKeyword]}", header_index,header_grandParent)]
                                    #else: # if element is the new keyword
                                        #if(grandParentAncestor is not None): # if grandparent and parent are under a different keyword
                                            #if(parentAncestor is not None): # if parent and grandparents are under different keywords
                                                #full_tag_path = header_dict[(f"{header}_{keyword_indexes[currentKeyword]}", header_level, f"{header_parent}_{keyword_indexes[parentAncestor]}", header_index,f"{header_grandParent}_{keyword_indexes[grandParentAncestor]}")]
                                            #else:
                                                #full_tag_path = header_dict[(f"{header}_{keyword_indexes[currentKeyword]}", header_level, f"{header_parent}_{keyword_indexes[parentAncestor]}", header_index,f"{header_grandParent}_{keyword_indexes[grandParentAncestor]}")]
                                        #else:
                                            #if(parentAncestor is not None): #if parent is under a different keyword, but grandparent not
                                                #full_tag_path = header_dict[(f"{header}_{keyword_indexes[currentKeyword]}", header_level, f"{header_parent}_{keyword_indexes[parentAncestor]}", header_index,header_grandParent)]
                                            #else:
                                                #full_tag_path = header_dict[(f"{header}_{keyword_indexes[currentKeyword]}", header_level, header_parent, header_index,header_grandParent)]
                                    #try: ##normal case
                                        ...
                                        #del header_dict[(f"{header}_{keyword_indexes[currentKeyword]}", header_level, f"{header_parent}_{keyword_indexes[currentKeyword]}", header_index)]
                                    #except: #when there is a keyword on the same level as another keyword
                                        #full_tag_path = header_dict[(f"{header}_{keyword_indexes[currentKeyword]}", header_level, f"{header_parent}_{keyword_indexes[parentTag]}", index)]
                                        #del header_dict[(f"{header}_{keyword_indexes[currentKeyword]}", header_level, f"{header_parent}_{keyword_indexes[currentKeyword]}", header_index)]
