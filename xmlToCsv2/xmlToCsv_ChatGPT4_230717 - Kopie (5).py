                    ...
                    if(header == "RefID" and itemTag != "TypeRoot" and header_level == 3):
                        ...
                        if(currentKeyword!= None):
                            try:
                                ...
                                #del header_dict[(f"{header}_{keyword_indexes[currentKeyword]}", header_level, f"{itemTag}_{keyword_indexes[currentKeyword]}", header_index)]
                            except:
                                try:
                                    full_tag_path = header_dict[(f"{header}_{keyword_indexes[currentKeyword]}", header_level, f"{itemTag}_{keyword_indexes[parentTag]}", index)]
                                    #del header_dict[(f"{header}_{keyword_indexes[currentKeyword]}", header_level, f"{itemTag}_{keyword_indexes[parentTag]}", index)]
                                except:
                                    continue
                        else:
                            try:
