...
def get_headers(root, parent_map): ####still some problems with saving all the headers - some branches have significantly more sub-branches, which have an impact on the headers amount too. The error might be here or with csv exporting. More troubleshooting necessary...
    ...
    for item in root.iter():
        ...
        for tag,level,pTag,gpTag in occurrence_counts.keys():
            if level > item_level:
                occurrence_counts[tag,level,pTag,gpTag] = 0
               

        # Increment the occurrence count for this tag, level, parent, and grandparent, and get the current count.
        occurrence_counts[(itemTag, item_level, parentTag,grandParentTag)] = occurrence_counts.get((itemTag, item_level, parentTag, grandParentTag), 0) + 1
        index = occurrence_counts[(itemTag, item_level, parentTag, grandParentTag)] - 1

        # Add the item to its parent's list of children in children_dict.
        children_dict.setdefault(parentTag, []).append((itemTag, item_level, parentTag, index, grandParentTag))
        ...
        for attr in item.attrib.keys():
            ...
            children_dict.setdefault(itemTag, []).append((attr, item_level, itemTag, attr_index, parentTag))
...
def get_first_row(headers, row, root,cell_path_matrix):
    ...
    for i, (header, header_level, header_parent, index, header_grandParent) in enumerate(headers):  # Manual correction to export also attributes of root
...
def write_lines (headers, root, cell_path_matrix, writer, parent_map): ##### trying to correct write_lines by introducing greatgrandparents into header_dict, but now having problems with accessing the dictionary when writing stuff out. Logic might need some deep rethinking. I created keyword_ancestor_dict to simplify the process
    ...
    for item in root.findall('.//'):
        ...
        for i, (header, header_level, header_parent, header_index, header_grandParent) in enumerate(headers):
            ...
            if (((header,header_parent) in data) or shortCategory) and (itemLevel == header_level):
                ...
                if(header != itemTag): #if we found attributes of item (which means level is the same as aprent)

                    try:
                        parentAncestor = keyword_ancestor_dict[(header_parent,header_level)]
                    except:
                        parentAncestor = None
                    try:
                        grandParentAncestor = keyword_ancestor_dict[(header_grandParent, header_level-1)]
                    except:
...
def get_keyword_ancestors(root, parent_map, keywords_tuple):
    ...
    def explore_node(node):
        ...
        keyword_ancestors[(node.tag, get_level(node, parent_map))] = has_keyword_ancestor
        for child in node:
            explore_node(child)
