...
def extract_xml_structure(node):
    if isinstance(node, ET.Element):
        tag = node.tag if "Idx" not in node.attrib else f"{node.tag}_{node.attrib['Idx']}"
        children = [extract_xml_structure(child) for child in node]
        has_text = True if node.text and not node.text.isspace() else False
        return (tag, node.attrib if node.attrib else None, children if children else None, has_text)
    else:
        return None
...
def build_header_dict(element, parent_map, parent_path="", header_dict=None): #I need to add a third element to the dictionary key, in order to differentiate the duplicates - we need to add this logic to cell_matrix too
    if header_dict is None:
        header_dict = {}

    level = get_level(element, parent_map)

    # If the tag is "Attributes", include its "Type" attribute in the tag name
    if element.tag == "Attributes" and "Type" in element.attrib:
        tag = f"{element.tag}_{element.attrib['Type']}" if element.attrib['Type'] else f"{element.tag}_EmptyType"
    else:
        tag = element.tag if "Idx" not in element.attrib else f"{element.tag}_{element.attrib['Idx']}"

    full_tag_path = f"{parent_path}/{tag}" if parent_path else tag

    header_dict[(tag, level)] = full_tag_path

    # Add attributes to the dictionary, except the "Type" attribute of an "Attributes" tag (already included in the tag)
    for attr in element.attrib:
        attr_path = f"{full_tag_path}@{attr}"
        header_dict[(attr, level)] = attr_path

        # If the attribute is "Type" or "Idx", also add an entry for the tag without the attribute
        if attr in ["Type", "Idx"]:
            original_tag = f"{element.tag}@{attr}"
            header_dict[(original_tag, level)] = f"{parent_path}/{original_tag}"
...
def get_headers(headers, header_tags, root, parent_map):
...
def xml_to_csv(xml_file, csv_file):
    ...
    with open(csv_file, 'w', newline='') as f:
        ...
        headers  = []
        header_tags = []
        headers, header_tags = get_headers(headers, header_tags, root, parent_map)
...
def navigate_to_element(element, tag_path_parts):
    """Navigate to a specified element in the XML tree."""
    current_element = element
    for tag in tag_path_parts:
        # Split the tag name by "_" to get the actual tag and its Idx or Type
        if("_" in tag):
            tag_parts = tag.split("_")
            actual_tag = tag_parts[0]
            idx_or_type = tag_parts[1] if len(tag_parts) > 1 else None
        else:
             actual_tag = tag
             idx_or_type = None
        
        if current_element.tag == actual_tag:
            continue

        if actual_tag == "Attributes" and idx_or_type:
            # For "Attributes" tag, we look for a child with the same tag and "Type" attribute
            matching_children = [child for child in current_element 
                                 if child.tag == actual_tag and child.get("Type", "") == idx_or_type]
        else:
            # For other tags, we look for a child with the same tag
            matching_children = [child for child in current_element if child.tag == actual_tag]

        if idx_or_type and idx_or_type.isdigit():
            # If idx_or_type is a number, use it as the index (Idx) of the child
            idx = int(idx_or_type)
            current_element = matching_children[idx-1]  # -1 because the index in XML starts from 1
        
        #if idx_or_type and idx_or_type.isalpha():
            #attr = str(idx_or_type)
            #for child in matching_children:
                #if()
        

        else:
            # Otherwise, just get the first matching child
            current_element = matching_children[0] if matching_children else None

    return current_element


def populate_xml_element(element, csv_row_index, csv_cell_index, row, cell_path_matrix): 
...
# Usage:
inputXML = "C:\\Users\\s.veress\\Desktop\\Linien.xml" #"Z:\\Mitarbeiter\\Szabolcs\\ArchicadVorlage\\AttributenStudie\\345_Linien.xml" Z:\\Mitarbeiter\\Szabolcs\\ArchicadVorlage\\Test.xml
